...
def initialiseVO(I) -> VO_state:
    ...
    kp0 = feature_detect(I[0], detector="harris")
...
def processFrame(I1, I0, S0: VO_state) -> Tuple[VO_state, Pose]:
    ...
    # print("ANGLE: ", np.degrees(np.arctan2(R_CW[2,0],R_CW[0,0])))
    ...
    candi_kp = feature_detect(I1, detector="SIFT")
...
def main() -> None:
    ...
    for img_idx, img_path in enumerate(DS_GLOB[STARTING_FRAME:]):
        ...
        state, T_CW = processFrame(frame, prev_frame, prev_state) # continuous VO markov chain

        prev_state = state # set current state to previous state for next image
        prev_frame = frame

        X_cam = T_CW @ prev_state.X

        n_front = np.sum(X_cam[2,:] > 0)
        print(f"2. Fraction points infront: {n_front/X_cam.shape[1]:.2f}, #pts: {prev_state.X.shape[1]}")
